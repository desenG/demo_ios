...
{
    NSLog(@"ContactTableViewController::addContact");
    inputContact=contact;
    BOOL isExistContact=false;
    for (Contact *Contact in mutablecontacts)
    {
        if (![contact.email  isEqual: @""]) {
            if ([Contact.email isEqualToString:contact.email]) {
                isExistContact = TRUE;
                Contact.ischeckd = @"yes";
                inputContact=Contact;
            }
        } else if (![contact.smsNumber isEqual: @""]) {
            if ([Contact.smsNumber isEqualToString:contact.smsNumber]) {
                isExistContact = TRUE;
                Contact.ischeckd = @"yes";
                inputContact=Contact;
            }
        }
        if (isExistContact) break;
    }
    
    if (!isExistContact)
    {
        NSLog(@"Contact Does not exist - add to array");
